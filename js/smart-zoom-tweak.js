...
(function( $ ){
  ...
  $.fn.smartZoom = function(method) {
    ...
    function moveOnMotion(xPos, yPos, duration, motionEnd){
    	...
    	//This is tweak for panning inside dragged image -- called a another target method with Pan
    	var validPosition = getValidTargetElementPositionForPan(newMarginLeft, newMarginTop, targetRect.width, targetRect.height); // check if the new position is valid
    ...
    /**
    	This method is tweaked to handle the zoom in dragged space
    ...
    function getValidTargetElementPosition(xPosition, yPosition, width, height){
	    ...
	    var newMarginTop = yPosition;//Math.min(smartData.adjustedPosInfos.top, yPosition); // adjust in top 
	    //newMarginTop += Math.max(0, (newMarginTop + height)) // adjust in bottom
	    var newMarginLeft =  xPosition; //Math.min(smartData.adjustedPosInfos.left, xPosition); // adjust in left
	    //newMarginLeft += Math.max(0, (newMarginLeft + width)); // adjust in right
	    return new Point(newMarginLeft.toFixed(2), newMarginTop.toFixed(2));
    }

    /**
    	This method is same as default getValidTargetElementPosition but used only for pannning inside dragged image. Will be called
    	only from mouseOnMotion
     * manage position validation
     * @param {Number} marginLeft : global x position
     * @param {Number} marginTop : global y position
     * @param {Number} width : element width
     * @param {Number} height : element height
     */
    function getValidTargetElementPositionForPan(xPosition, yPosition, width, height){
    	var smartData = targetElement.data('smartZoomData');
    	 // adjusting if the content is out of the initial content box from "adjustedPosInfos"
