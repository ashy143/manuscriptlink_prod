 ...
 */
    ...
    if($res->num_rows < 1){
        ...
        //$image_path = IMAGE_UPLOAD_ROOT_DIR . join(DIRECTORY_SEPARATOR, array($_POST['image_institution'], $_POST['image_collection'], $_POST['image_shelfmark'], $_POST['image_name'].'.jpg'));
        $uploaddir = IMAGE_UPLOAD_ROOT_DIR . join(DIRECTORY_SEPARATOR, array($_POST['image_institution'], $_POST['image_collection'], $_POST['image_shelfmark']));
        $image_path = $uploaddir . '/' . $_POST['image_name'].'.jpg';     

        $width_written_space_broken = 0;
        if(isset($_POST['width_written_space_broken'])){
            $width_written_space_broken = 1;
        }
        $ht_written_space_broken = 0;
        if(isset($_POST['width_written_space_broken'])){
            $width_written_space_broken = 1;
        }
        $no_of_lines_broken = 0;
        if(isset($_POST['ht_written_space_broken'])){
            $ht_written_space_broken = 1;
        }
        $ht_fol_broken = 0;
        if(isset($_POST['ht_fol_broken'])){
            $ht_fol_broken = 1;
        }
        $width_fol_broken = 0;
        if(isset($_POST['width_fol_broken'])){
            $width_fol_broken = 1;
        }
        $staves_per_page_broken = 0;
        if(isset($_POST['staves_per_page_broken'])){
            $staves_per_page_broken = 1;
        }
                ...
                if ($_FILES["file"]["error"] > 0){
                    ...
                    error_log(IMAGE_UPLOAD_ROOT_DIR . $_FILES["file"]["name"]);
                      ...
                      if (file_exists(IMAGE_UPLOAD_ROOT_DIR . $_FILES["file"]["name"])){
                        ...
                        createPath($uploaddir);
                        // if (!is_dir($uploaddir) && !mkdir($uploaddir)){
                        //       die("Error creating folder $uploaddir");
                        // }
                        copy($_FILES["file"]["tmp_name"], $image_path);
        ...
        $query = sprintf("INSERT INTO folios_temp values 
            ...
            $_POST['lines_per_staff'], $_POST['dim_staff'], mysqli_real_escape_string($mysqli, $_POST['col_staff']), mysqli_real_escape_string($mysqli, $image_path), mysqli_real_escape_string($mysqli, $_POST['website']), 
            ...
            $no_of_lines_broken, $ht_fol_broken, $width_fol_broken, $ht_written_space_broken, $width_written_space_broken,
            $staves_per_page_broken, $min_ht, $max_ht, $max_width, $min_width,
            $min_ht_written, $max_ht_written, $min_width_written, $max_width_written, $_POST['ref_folio_id'] );
